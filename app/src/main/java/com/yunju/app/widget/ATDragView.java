...
public class ATDragView extends View {
    private static final float SEEK_BG_SCALE = 1.1F / 3.5F;
    private static final float SEEK_TEXT_SCALE = 1.F / 1.5F;
...
//            float measureTextWidth = seekTextPaint.measureText(tempDesc);
            Rect rect = new Rect();
            seekTextPaint.getTextBounds(tempDesc, 0, 1, rect);
            canvas.drawText(tempDesc, DEF_PADDING + unitWidth * i- rect.width()/2, seekTextY, seekTextPaint);
        }
    }
