...
public class MenuDialogAdapter extends BaseAdapter {
    ...
    private List<FilterData> menuDatas;
    ...
    public MenuDialogAdapter(Context mContext, List<FilterData> menuDatas) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        final FilterData menuData = menuDatas.get(position);
        nameText.setText(menuData.getItemText());//设置标题

        convertView.setSelected(selectedPos == position);//设置选中时的view
        nameText.setSelected(selectedPos == position);

        //选中后的标题字体颜色
        nameText.setTextColor(selectedPos == position ? mContext.getResources().getColor(R.color.colorPrimary) : 0xFF333333);
        //选中与未选中的背景色
